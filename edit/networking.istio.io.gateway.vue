...
export default {
  ...
  async fetch() {
    const receivers = this.$store.dispatch("cluster/findAll", {
      type: MONITORING.SPOOFED.RECEIVER,
    });
    const routes = this.$store.dispatch("cluster/findAll", {
      type: MONITORING.SPOOFED.ROUTE,
    });

    this.receiverOptions = (await receivers).map(
      (receiver) => receiver.spec.name
    );
    ...
    if (this.isCreate) {
      ...
      this.$set(
        this.value.spec,
        "name",
        createDefaultRouteName(nonRootRoutes.length)
      );
    }
  },
